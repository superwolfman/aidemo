...
export function RuntimeSummary({ activeRun, blueprint, ragRuntime, ragLive }: Props) {
  ...
  const taskMode = activeRun?.executionContext?.taskMode;
  return (
    <section className="ops-runtime-summary panel">
      <div className="ops-runtime-title">
        <span>当前选中 Run</span>
        <h2>{taskMode?.label || activeRun?.intent?.label || '等待 Agent Run'}</h2>
        {activeRun ? <strong>执行 Agent：{activeRun.selectedSkill?.name || 'Runtime Router'}{taskMode ? ` · 意图：${activeRun.intent?.label || '未识别'}` : ''}</strong> : null}
