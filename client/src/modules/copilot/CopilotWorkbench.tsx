...
export default function CopilotWorkbench() {
    ...
    const {
        ...
        approval: approvalState, setApproval, approvalHistory, setApprovalHistory,
    ...
    const { approval: activeApproval, draftRevision, setDraftRevision, review: reviewApproval, receive } = approval;
    ...
    const [artifactReviews, setArtifactReviews] = useState<Record<string, any>>({});
    ...
    const selectedWorkflowSlot = useMemo(() => productWorkflowSlots.find((slot) => slot.type === selectedWorkflowArtifactType) || productWorkflowSlots[0], [selectedWorkflowArtifactType]);
    const selectedWorkflowArtifact = useMemo(() => artifacts.find((artifact) => artifact.type === selectedWorkflowSlot.type), [artifacts, selectedWorkflowSlot.type]);
    const selectedWorkflowReview = selectedWorkflowArtifact ? artifactReviews[selectedWorkflowArtifact.id] : null;
    const selectedWorkflowContent = selectedWorkflowArtifact ? selectedWorkflowReview?.draft || artifactToText(selectedWorkflowArtifact) : '';
    ...
    const activeTemplatePacks = templates.filter((template) => template.tags?.some((tag) => activeScopes.includes(tag) || tag === 'copilot')).slice(0, 4);
    ...
    const evalChecks = useMemo(() => ({ citations: sources.length > 0, prd: artifacts.some((a) => a.type === 'prd'), api: artifacts.some((a) => a.type === 'api'), trace: trace.length >= 6 }), [artifacts, trace, sources]);
    const pendingWorkflowSummary = useMemo(() => {
        const fields = activeMode.fields.map((field) => ({ label: field.label, value: form[field.key] || field.placeholder })).filter((item) => item.value);
        return { title: selectedEvalCase?.title || activeMode.label, fields };
    ...
    async function createSessionLocal() {
        const result = await sessionService.createCopilotSession(`${activeMode.label}会话`, skillId);
        setSessions((items) => [result.session, ...items]); setActive(result.session);
        setTrace([]); setRunState({ status: 'idle', label: '等待输入' }); setReplayIndex(null); setSources([]); setArtifacts([]); setArtifactReviews({}); setApproval(null); setRagPreview([]);
    ...
    async function sendPrompt(nextPrompt = prompt) {
        ...
        try {
            await copilotService.streamCopilotMessage(
                active._id,
                {
                    prompt: nextPrompt,
                    skillId,
                    ...
                    delta: (payload) => {
                        setActive((current) => current ? {
                            ...
                            messages: current.messages.map((m) =>
                                m.id === assistant.id ? { ...m, content: m.content + payload.text } : m
                            )
                        } : current);
                    },
                    approval: (payload) => receive(payload),
                    final: async () => { await refreshActive(active._id); }
                },
                controller.signal
            );
        } catch (error) {
    ...
    function stop() { abortRef.current?.abort(); setRunning(false); setRunState({ status: 'cancelled', label: '用户已停止生成' }); }
    function regenerate() { const lastUser = [...messages].reverse().find((m) => m.role === 'user'); sendPrompt(lastUser?.content || prompt); }
    function editAsPrompt(content: string) { setPrompt(content); requestAnimationFrame(() => { document.querySelector('.chat-composer')?.scrollIntoView({ behavior: 'smooth', block: 'center' }); }); }
    function updateArtifactReview(id: string, patch: any) { setArtifactReviews((current) => { const base = current[id] || { status: 'draft', version: 1, draft: '', history: [{ version: 1, status: 'created', at: new Date().toISOString() }] }; return { ...current, [id]: { ...base, ...patch } }; }); }
    function saveArtifactDraft(artifact: Artifact) { const current = artifactReviews[artifact.id]; if (!current) return; const version = current.version + 1; updateArtifactReview(artifact.id, { status: 'draft', version, history: [{ version, status: 'edited', at: new Date().toISOString() }, ...current.history].slice(0, 6) }); }
    function confirmArtifact(artifact: Artifact) { const current = artifactReviews[artifact.id]; const version = current?.version || 1; updateArtifactReview(artifact.id, { status: 'confirmed', history: [{ version, status: 'confirmed', at: new Date().toISOString() }, ...(current?.history || [])].slice(0, 6) }); }
    function exportArtifact(artifact: Artifact, format: 'md' | 'json') { const review = artifactReviews[artifact.id]; const content = review?.draft || artifactToText(artifact); if (format === 'json') { downloadText(`${artifact.type}-${artifact.id}.json`, JSON.stringify({ id: artifact.id, type: artifact.type, title: artifact.title, version: review?.version || 1, status: review?.status || 'draft', content }, null, 2), 'application/json;charset=utf-8'); return; } downloadText(`${artifact.type}-${artifact.id}.md`, artifactToMarkdown(artifact, content), 'text/markdown;charset=utf-8'); }
    function changeMode(mode: TaskMode) { setSelectedEvalCase(null); setTaskModeId(mode.id); setSkillId(mode.skillId); setPrompt(mode.prompt); setForm(Object.fromEntries(mode.fields.map((field) => [field.key, '']))); }
    function selectSkill(skill: Skill) { const mode = taskModes.find((item) => item.skillId === skill.id); if (mode) { changeMode(mode); return; } setSkillId(skill.id); }
    function selectSession(session: Session) { setSelectedEvalCase(null); setActive(session); setTrace([]); setRunState({ status: 'idle', label: '等待输入' }); setReplayIndex(null); setSources([]); setArtifacts([]); setArtifactReviews({}); setApproval(null); setRagPreview([]); setRagDiagnostics(runtime ? { rag: runtime.rag, query: '', scopes: [], sources: [], source: 'runtime' } : null); const mode = taskModes.find((item) => item.skillId === session.activeSkillId); if (mode) { setTaskModeId(mode.id); setSkillId(mode.skillId); setPrompt(mode.prompt); setForm(Object.fromEntries(mode.fields.map((field) => [field.key, '']))); } }
    function applyScenario(scenario: typeof demoScenarios[number]) { const mode = taskModes.find((item) => item.id === scenario.modeId) || taskModes[0]; setSelectedEvalCase(null); setTaskModeId(mode.id); setSkillId(mode.skillId); setPrompt(mode.prompt); setForm(scenario.form); setArtifacts([]); setArtifactReviews({}); setTrace([]); setRunState({ status: 'idle', label: '等待输入' }); setSources([]); setApproval(null); setRagPreview([]); setRagDiagnostics(runtime ? { rag: runtime.rag, query: '', scopes: [], sources: [], source: 'runtime' } : null); }
    async function applyEvalCase(item: EvalCase) { const mode = taskModes.find((task) => task.id === item.modeId) || taskModes[0]; setSelectedEvalCase(item); setTaskModeId(mode.id); setSkillId(mode.skillId); setPrompt(mode.prompt); setForm(item.form); const cleanSessionTitle = `${item.title} Eval 会话`; try { const created = await sessionService.createCopilotSession(cleanSessionTitle, mode.skillId); setSessions((items) => [created.session, ...items.filter((s) => s._id !== created.session._id)]); setActive(created.session); } catch { setActive((current) => current ? { ...current, title: cleanSessionTitle, activeSkillId: mode.skillId, messages: [] } : current); } setArtifacts([]); setArtifactReviews({}); setTrace([]); setSources([]); setApproval(null); setRunState({ status: 'idle', label: `已加载 Eval Case：${item.title}` }); setRagPreview([]); setRagDiagnostics(runtime ? { rag: runtime.rag, query: '', scopes: [], sources: [], source: 'runtime' } : null); }
    function buildPromptFromMode() { const fieldText = activeMode.fields.map((field) => `${field.label}: ${form[field.key] || field.placeholder}`).join('\n'); return `${activeMode.prompt}\n\n${fieldText}`; }
    async function uploadKnowledgeFile(file?: File) { if (!file) return; const isPdf = file.name.toLowerCase().endsWith('.pdf'); const content = isPdf ? '' : await file.text(); await uploadKnowledge({ filename: file.name, mimeType: file.type, content, scopes: activeSkill?.knowledgeScopes || ['architecture'] }); }
    async function importTemplatePack(templateId: string) { await importTemplate(templateId); }
    async function syncProjectKnowledgeLocal() { await syncProject(); }
    async function checkVectorStoreLocal() { await checkVectorStore(); }
    async function searchKnowledgePreview() { const nextQuery = ragQuery || buildPromptFromMode(); await searchPreview(nextQuery, activeScopes); }
    async function reviewApprovalLocal(action: 'confirm' | 'revise' | 'reject') { await reviewApproval(action, draftRevision, action); }
    function restoreTracePath() { const traceFromMessage = [...messages].reverse().find((m) => m.trace?.length)?.trace || trace; setTrace(traceFromMessage); setReplayIndex(traceFromMessage.length ? 0 : null); setOpenTraceIds(traceFromMessage[0]?.id ? [traceFromMessage[0].id] : []); }
    function nextReplayStep() { if (replayIndex === null) return; setReplayIndex((index) => { const next = Math.min((index || 0) + 1, trace.length - 1); if (trace[next]?.id) setOpenTraceIds((ids) => ids.includes(trace[next].id) ? ids : [...ids, trace[next].id]); return Number.isFinite(next) ? next : null; }); }
